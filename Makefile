...
# Print file names followed by file content.
	...
	diff -x tools -x defs.tex -x git -x merges -x .gitignore -x git -x result.csv -x plots -x filtered_result.csv -x table_run_time.tex -x .DS_Store -x '*~' -r -U3 test/small-goal-files results-small
